...
const renderers={title:renderTitle,equation:renderEquation,groups:renderGroups,array:renderArray,numberLine:renderNumberLine,summary:renderSummary,composition:renderComposition};
const elementRenderers={text:elementText,panel:elementPanel,visual:elementVisual,equation:elementEquation,grid:elementGrid,groups:elementGroups,numberLine:elementNumberLine,list:elementList};
const clamp=(v,a=0,b=1)=>Math.max(a,Math.min(b,v)),ease=t=>1-Math.pow(1-clamp(t),3),wave=(t,s=1)=>Math.sin(t*Math.PI*2*s),spring=t=>1-Math.exp(-7*clamp(t))*Math.cos(12*clamp(t));
...
function fit(){canvas.width=data.video.width||1920;canvas.height=data.video.height||1080;ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality="high"}
function text(v,x,y,size,color,weight="600",align="center",font=theme().font){ctx.fillStyle=color;ctx.font=`${weight} ${size}px "${font}"`;ctx.textAlign=align;ctx.textBaseline="middle";ctx.fillText(String(v),x,y)}
function rounded(x,y,w,h,r,fill,alpha=1,stroke=null){let base=ctx.globalAlpha;ctx.globalAlpha=base*alpha;ctx.fillStyle=fill;ctx.beginPath();ctx.roundRect(x,y,w,h,r);ctx.fill();if(stroke){ctx.strokeStyle=stroke;ctx.lineWidth=2;ctx.stroke()}ctx.globalAlpha=base}
...
function background(t=0){let w=canvas.width,h=canvas.height,g=ctx.createLinearGradient(0,0,w,h);g.addColorStop(0,data.video.background||"#061426");g.addColorStop(1,"#10243f");ctx.fillStyle=g;ctx.fillRect(0,0,w,h);[[w*.18+wave(t,.04)*80,h*.15,theme().primary],[w*.83,h*.8+wave(t,.06)*70,theme().accent]].forEach(([x,y,c])=>{let glow=ctx.createRadialGradient(x,y,0,x,y,w*.48);glow.addColorStop(0,c);glow.addColorStop(1,"transparent");ctx.save();ctx.globalAlpha=.16;ctx.fillStyle=glow;ctx.fillRect(0,0,w,h);ctx.restore()});ctx.globalAlpha=.055;ctx.strokeStyle=theme().primary;for(let x=-h;x<w+h;x+=96){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x+h,h);ctx.stroke()}ctx.globalAlpha=1}
...
function svgObject(o,p){let a=asset(o.asset);if(!a)return;let vb=parseViewBox(a.viewBox),w=o.width||120,h=o.height||120,x=o.x||0,y=o.y||0;ctx.save();animateTransform(o,p,x,y,w,h);ctx.translate(-w/2,-h/2);ctx.scale(w/vb.w,h/vb.h);ctx.translate(-vb.x,-vb.y);let base=ctx.globalAlpha;for(const part of a.paths||[]){let path=new Path2D(part.d);ctx.globalAlpha=base*(part.opacity??1);ctx.lineWidth=part.strokeWidth||1;ctx.lineCap=part.lineCap||"butt";ctx.lineJoin=part.lineJoin||"miter";if(part.fill&&part.fill!=="none"){ctx.fillStyle=part.fill;ctx.fill(path)}if(part.stroke){ctx.strokeStyle=part.stroke;ctx.stroke(path)}}ctx.restore();ctx.globalAlpha=1}
function animateTransform(o,p,x,y,w=0,h=0){let d=o.delay||0,q=ease(clamp((p-d)/(1-d))),anim=o.animation||"fade";ctx.globalAlpha=q;ctx.translate(x,y);if(anim==="pulse"){let z=1+.1*wave(p,1.4);ctx.scale(z,z)}else if(anim==="bounce"||anim==="hop")ctx.translate(0,-Math.abs(wave(p,1.5))*45);else if(anim==="float")ctx.translate(0,wave(p,.7)*22);else if(anim==="wave")ctx.translate(0,wave(p+d,1.2)*16);else if(anim==="pop"){let z=.2+.8*q;ctx.scale(z,z)}else if(anim==="spinIn"){ctx.rotate((1-q)*-Math.PI);ctx.scale(q,q)}else if(anim==="slideLoop")ctx.translate(wave(p,.75)*45,0);else if(anim==="draw")ctx.rotate((o.spin||0)*p*Math.PI*2)}
function visual(o,p,defaults={}){o={...defaults,...o};if(!o)return;let x=o.x||0,y=o.y||0;if(o.kind==="svg")return svgObject(o,p);ctx.save();animateTransform(o,p,x,y);ctx.shadowColor=o.color||theme().primary;ctx.shadowBlur=18;ctx.shadowOffsetY=5;let font=o.kind==="emoji"?theme().emojiFont:theme().iconFont;text(o.value||"",0,0,o.size||70,o.color||theme().text,"700","center",font);ctx.restore()}
function decorations(s,p){for(const d of s.decorations||[])visual(d,p)}
// A composition scene is a canvas layout assembled from reusable elements.
// Every element owns its position, delay, animation and z-index.
function renderComposition(s,p){for(const e of (s.elements||[]).slice().sort((a,b)=>(a.z||0)-(b.z||0)))renderElement(e,p);decorations(s,p)}
function renderElement(e,p){let fn=elementRenderers[e.type];if(!fn)return;let q=ease(clamp((p-(e.delay||0))/(e.enterDuration||.35)));ctx.save();ctx.globalAlpha=q;ctx.translate(e.x||0,e.y||0);if(e.animation==="pop"){let z=.7+.3*spring(q);ctx.scale(z,z)}if(e.animation==="float")ctx.translate(0,wave(p,.6)*12);if(e.animation==="pulse"){let z=1+wave(p,1.2)*.035;ctx.scale(z,z)}if(e.animation==="slideLeft")ctx.translate((1-q)*80,0);fn(e,p,q);ctx.restore()}
function elementText(e){if(e.maxWidth)wrap(e.content??e.text,0,0,e.maxWidth,e.size||42,e.color||theme().text,e.lineHeight||1.25);else text(e.content??e.text,0,0,e.size||42,e.color||theme().text,e.weight||"600",e.align||"center")}
function elementPanel(e){let w=e.width||400,h=e.height||180;ctx.shadowColor="#0008";ctx.shadowBlur=30;ctx.shadowOffsetY=14;rounded(-w/2,-h/2,w,h,e.radius||24,e.color||theme().panel,e.opacity??.94,e.border||theme().primary+"55")}
function elementVisual(e,p){visual({...e,x:0,y:0},p)}
function elementEquation(e){text(e.content||e.equation||"",0,0,e.size||84,e.color||theme().secondary,e.weight||"800")}
function elementGrid(e,p){let rows=e.rows||3,cols=e.columns||3,cell=e.cellSize||Math.min(120,620/Math.max(rows,cols));for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){let i=r*cols+c;visual({kind:"fontIcon",value:"●",size:e.size||cell*.62,color:i%2?(e.alternateColor||theme().accent):(e.color||theme().primary),animation:e.itemAnimation||"pop",delay:(e.delay||0)+i/(rows*cols)*.45,x:(c-(cols-1)/2)*cell,y:(r-(rows-1)/2)*cell},p)}}
function elementGroups(e,p){let groups=e.groups||e.count||3,count=e.itemsPerGroup||3,gap=(e.width||1100)/groups;for(let g=0;g<groups;g++){let gx=(g-(groups-1)/2)*gap;rounded(gx-gap*.4,-170,gap*.8,340,26,e.panelColor||theme().panel,.9);for(let i=0;i<count;i++)visual({kind:"fontIcon",value:"●",size:56,color:i%2?theme().secondary:theme().primary,...e.item,x:gx-68+(i%2)*136,y:-55+Math.floor(i/2)*112,delay:(e.delay||0)+(g*count+i)/(groups*count)*.45},p)}}
function elementNumberLine(e,p){let jumps=e.jumps||3,from=e.from||0,step=e.step||1,w=e.width||1200,x0=-w/2,x1=w/2;ctx.strokeStyle=theme().text;ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(x0,0);ctx.lineTo(x1,0);ctx.stroke();for(let i=0;i<=jumps;i++){let x=x0+w*i/jumps;ctx.fillStyle=theme().secondary;ctx.fillRect(x-5,-20,10,40);text(from+i*step,x,62,32,theme().text,"700")}let done=ease(p)*jumps;for(let i=0;i<Math.ceil(done);i++){let a=x0+w*i/jumps,b=x0+w*(i+clamp(done-i))/jumps;ctx.strokeStyle=theme().primary;ctx.lineWidth=13;ctx.beginPath();ctx.arc((a+b)/2,-60,(b-a)/2,Math.PI,0);ctx.stroke()}visual({...e.jumper,x:x0+w*done/jumps,y:-110},p)}
function elementList(e,p){let items=e.items||[],w=e.width||900;items.forEach((v,i)=>{let o=typeof v==="string"?{text:v}:v,y=(i-(items.length-1)/2)*(e.gap||100),q=ease(clamp((p-(e.delay||0)-i*.1)/.3));rounded(-w/2,y-36,w,72,16,theme().panel,q);visual({kind:"fontIcon",value:"✓",color:theme().primary,size:34,...o.icon,x:-w/2+34,y,delay:(e.delay||0)+i*.1},p);text(o.text||"",-w/2+72,y,e.size||34,e.color||theme().text,"600","left")})}
...
function render(t){background(t);let s=activeScene(t);if(!s)return;let p=clamp((t-s.start)/s.duration);ctx.save();transition(s,p);(renderers[s.type]||renderTitle)(s,p);ctx.restore();ctx.globalAlpha=1;ctx.shadowColor="transparent";ctx.shadowBlur=0}
...
function validate(o){if(!o?.video||!Array.isArray(o.scenes)||!o.scenes.length)throw Error("JSON requires video and scenes");for(const s of o.scenes){if(!renderers[s.type])throw Error(`Unsupported scene type: ${s.type}`);if(s.type==="composition"&&!Array.isArray(s.elements))throw Error("Composition scenes require an elements array");for(const e of s.elements||[])if(!elementRenderers[e.type])throw Error(`Unsupported element type: ${e.type}`)}}
