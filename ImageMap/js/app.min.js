!function (a) {
    var b = 0
        ...
        , d = {
            ...
            ConvertToCSV: function (filename, objArray) {
            ...
            asJSON: function (b) {
                ...
                c[b].options.file.length > 0 && (e = c[b].options.file),
                    ds.attr("src", e),
                    ds.attr("usemap", "#image-map");
                var f = a("<map />");
                f.attr("name", "image-map");
                var g = [];
                results = {};
                results.image = { id: 1, imageUrl: e, }
                var result = [];
                a.each(c[b].area, function (d, e) {
                    ...
                    var i = a("<area />");
                    var itemRow = {};
                    itemRow.target = c[b].area[d].target;
                    itemRow.alt = c[b].area[d].title;
                    itemRow.title = c[b].area[d].title;
                    itemRow.href = c[b].area[d].href;
                    itemRow.coords = f.join(",");
                    itemRow.shape = c[b].area[d].shape;
                    // result.push(itemRow);
                    ...
                    result.push({ html: i[0].outerHTML, value: c[b].area[d].title, imageId: 1 });
                }),
                    f.append("\n    " + g.join("\n    ") + "\n");
                results.result = result;
                console.log(results);
                var h = "<!-- Image Map Generated by http://www.image-map.net/ -->";
                d.ConvertToCSV("imageInfo", [results.image])
                return h + "\n" + ds[0].outerHTML + "\n\n" + d.ConvertToCSV("AreaHTml", results.result)
            },
            asHTML: function (b) {
                var d = a("<img />")
                    , e = c[b].options.src;
    ...
    a.fn.imageMapper = function () {
        ...
        return "getData" == b ? d.getData(f) : "asHTML" == b ? d.asHTML(f) : "asJSON" == b ? d.asJSON(f) : (e.unshift(f),
            this.each(function () {
        ...
        a(document).on("init", function () {
            ...
            a(document).on("click", "button.add-row", function (b) {
                ...
                a(document).on("show.bs.modal", "#modal-code", function () {
                    var b = a("#image-map").imageMapper("asJSON");
